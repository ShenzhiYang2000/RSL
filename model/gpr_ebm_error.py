...
def init_weights(m):
    if isinstance(m, nn.Linear):
        # nn.init.normal_(m.weight, mean=0.0, std=0.02)  # 正态分布初始化权重
        # nn.init.kaiming_normal_(m.weight, nonlinearity='relu')  # 适合 ReLU 激活函数
...
class MLP(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout_rate=0.2):
        ...
        self.fc3 = nn.Linear(2, out_channels)
        self.dropout_rate = dropout_rate
...
class GPR_EBM(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, dropout_adj, sparse, num_classes):
        ...
        #ETF Initialization
        P = self.generate_random_orthogonal_matrix(2, num_classes,try_assert = True)
    ...
    def forward_ETF(self, x, g, dropout_rate=0.0, return_x = False):
        # x = self.BN_H(x)
